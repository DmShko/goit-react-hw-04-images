...
import getDataFromApi from 'getFromAPI';
...
export const App = () => {
  ...
  const [fillingLevel, setFillingLevel] = useState(
    Math.floor(totalH / quantityCard)
  );
  ...
  const reset = () => {
    ...
    const hooksFilter = Object.keys(hooks).filter(
      value => value !== 'inputData' && value !== 'checkData'
    );
  ...
  useEffect(() => {
    if (pageCounter !== 0) {
      ...
      if (pageCounter === fillingLevel && totalH > quantityCard) {
